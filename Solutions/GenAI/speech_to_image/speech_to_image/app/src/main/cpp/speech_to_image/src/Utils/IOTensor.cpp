...
iotensor::PopulateInputTensorsRetType_t iotensor::IOTensor::populateInputTensor(uint32_t graphIdx,
    Qnn_Tensor_t* input,
  ...
  if(graphIdx == 1){//unet
    if(QNN_TENSOR_GET_RANK(input) == 4){
      float random_gen[16384];
      datautil::generateNormalNumber(0.0, 1.0, 16384, random_gen);
      returnStatus = copyFromFloatToNative(random_gen, input);
    }
  }
  else if(graphIdx == 0){//text_encoder
    std::string text = "tokens.raw";
    std::vector<int32_t> tokensId = datautil::tokenize(text);
     //returnStatus = copyFromFloatToNative(tokensId.data(), input);
     int8_t * pointerToBuffer = static_cast<int8_t*>(QNN_TENSOR_GET_CLIENT_BUF(input).data);
     int8_t * pointertoInputData = reinterpret_cast<int8_t*>(tokensId.data());
     for(size_t i=0; i<QNN_TENSOR_GET_CLIENT_BUF(input).dataSize; i++){
        pointerToBuffer[i] = pointertoInputData[i];
     }
  }else if(graphIdx == models::ENCODER){
    std::vector<float> inp_data = log_mel(audio_path, cache_path);

    float * input_tensor_encoder = reinterpret_cast<float*>(QNN_TENSOR_GET_CLIENT_BUF(input).data);

    for(size_t i=0; i<QNN_TENSOR_GET_CLIENT_BUF(input).dataSize / sizeof(float); i++){
      input_tensor_encoder[i] = inp_data[i];
    }
  }else if(graphIdx == models::DECODER){
    int32_t * input_tensor_decoder;

    switch (node_index){
    case decoder_input::INPUT_IDS:
      input_tensor_decoder = reinterpret_cast<int32_t*>(QNN_TENSOR_GET_CLIENT_BUF(input).data);
      input_tensor_decoder[0] = START_TOKEN;
      break;
    case decoder_input::POSITION_IDS:
      input_tensor_decoder = reinterpret_cast<int32_t*>(QNN_TENSOR_GET_CLIENT_BUF(input).data);
      input_tensor_decoder[0] = 0;
      break;
    default:
      break;
    }
  }
  return {returnStatus, numFilesPopulated};
}
...
iotensor::PopulateInputTensorsRetType_t iotensor::IOTensor::populateInputTensors(
    ...
    iotensor::InputDataType inputDataType) {
  //QNN_INFO("populateInputTensors() graphIndx %d", graphIdx);
  if (nullptr == inputs) {
  ...
  for (size_t inputIdx = 0; inputIdx < inputCount; inputIdx++) {
    ...
    std::tie(returnStatus, currentInputNumFilesPopulated) =
        populateInputTensor(graphIdx,
                            &(inputs[inputIdx]),
                            inputDataType,
                            inputIdx);
    if (StatusCode::SUCCESS != returnStatus) {
      QNN_ERROR("populateInputTensor failed for input %s with index %d",
                inputNodeName.c_str(),
                inputIdx);
      return {StatusCode::FAILURE, currentInputNumFilesPopulated};
    }
  }
  return {StatusCode::SUCCESS, numFilesPopulated};
}
...
iotensor::StatusCode iotensor::IOTensor::setupTensors(Qnn_Tensor_t** tensors,
  ...
  *tensors          = (Qnn_Tensor_t*)calloc(1, tensorCount * sizeof(Qnn_Tensor_t));
  ...
  for (size_t tensorIdx = 0; tensorIdx < tensorCount; tensorIdx++) {
    ...
    uint32_t* dimensions =  QNN_TENSOR_GET_DIMENSIONS(wrapperTensor);

    QNN_INFO("speech_to_image -> Tensor name %s", QNN_TENSOR_GET_NAME(wrapperTensor));
    QNN_INFO("speech_to_image -> Tensor rank %d", QNN_TENSOR_GET_RANK(wrapperTensor));
    for(size_t i=0; i<QNN_TENSOR_GET_RANK(wrapperTensor); i++){
      QNN_INFO("speech_to_image -> Dimension: %d: %d", i, *(dimensions + i));
    }
    QNN_INFO("speech_to_image -> Tensor data type: %d", QNN_TENSOR_GET_DATA_TYPE(wrapperTensor));
    QNN_INFO("speech_to_image -> Tensor tensor ID : %d", QNN_TENSOR_GET_ID(wrapperTensor));
    ...
    if (StatusCode::SUCCESS == returnStatus) {
      ...
      returnStatus =
          (speech_to_image::deepCopyQnnTensorInfo(((*tensors) + tensorIdx), &wrapperTensor) == true
               ? StatusCode::SUCCESS
               : StatusCode::FAILURE);
    }
    if (StatusCode::SUCCESS == returnStatus) {
...
iotensor::StatusCode iotensor::IOTensor::allocateBuffer(uint8_t** buffer,
  ...
  switch (dataType) {
    case QNN_DATATYPE_FLOAT_16:
      QNN_DEBUG("allocating float16 (uint16_t) buffer");
