...
public class BitmapUtility {
    ...
    public long countGreenPixel(int dim, final byte[] inputArray){
        final float inputScale = 0.00392156862745f;

        long sumGreenValues = 0;
        int srcIndex = 0, dstIndex = 0;
        for (int i = 0; i < dim; i++) {
            final int tempRedPixel = inputArray[srcIndex] & 0xFF;
            tempFloatBuffer[dstIndex + 2] = inputScale * (float) tempRedPixel;

            final int tempGreenPixel = inputArray[srcIndex + 1] & 0xFF;
            tempFloatBuffer[dstIndex + 1] = inputScale * (float) tempGreenPixel;

            final int tempBluePixel = inputArray[srcIndex + 2] & 0xFF;
            tempFloatBuffer[dstIndex] = inputScale * (float) tempBluePixel;
...
//        System.out.println("BufferBlack: "+mIsBufferBlack);
//        System.out.println("sumGreenValues "+sumGreenValues);
//        int tdim = dim*13;
//        System.out.println("dim: "+tdim);
